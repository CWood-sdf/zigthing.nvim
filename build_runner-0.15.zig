...
fn runStepNames(
    ...
    const filePath = try std.process.getEnvVarOwned(gpa, "ERRORFILE_PATH");
    defer gpa.free(filePath);
    const errFile = try std.fs.createFileAbsolute(
        filePath,
        .{ .truncate = true },
    );
    defer errFile.close();
    ...
    defer errFileWriter.flush() catch {};
...
fn workerMakeOneStep(
    ...
    if (show_error_msgs or show_compile_errors or show_stderr) {
        ...
        const options2: std.zig.ErrorBundle.RenderOptions = .{
            .ttyconf = .no_color,
            .include_reference_trace = (b.reference_trace orelse 0) > 0,
        };
        printErrorMessages(run.gpa, s, options2, errFileWriter, run.prominent_compile_errors) catch {};
    }
    ...
    handle_result: {
        ...
        for (s.dependants.items) |dep| {
            thread_pool.spawnWg(wg, workerMakeOneStep, .{
                wg, b, dep, prog_node, run, errFileWriter,
            });
        }
    }
