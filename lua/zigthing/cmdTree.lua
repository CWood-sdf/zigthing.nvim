...
function M.validateSubTree(tree, path)
    ...
    for k, v in pairs(tree) do
        ...
        if k == "_callback" then
            assert(
                ...
                .. type(v)
                .. ", but expected a function) (at path "
                .. path
                .. ")"
            )
            goto continue
    ...
    for k, v in ipairs(tree) do
        ...
        assert(
            v.tp == "repeat" or v.tp == "optional" or v.tp == "required" or v.tp == "positional" or v.tp == "flag",
        ...
        if v.tp == "optional" then
            assert(
                k == #tree or (k == #tree - 1 and tree[#tree].tp == "flag"),
                "Invalid cmd tree, optional parameter must be last or last before flags (" .. location .. ")"
            )
        end
        if v.tp == "flag" then
            flagCount = flagCount + 1
            assert(k == #tree, "Invalid cmd tree, flag parameter set must be last (" .. location .. ")")
        end
        if flagCount > 1 then
